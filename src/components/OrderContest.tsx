...
export default function OrderContest({order} : OrderContestProps) {
  return (
    <div>
            ...
            :(
                order.map( item => (
                    <div key={item.id} className="flex justify-between hover:bg-teal-100 items-center px-3 py-2 border-t border-gray-300 last-of-type:border-b">
                        ...
                        <button className="bg-red-600 h-8 w-8 rounded-full text-white font-black cursor-pointer">x</button>
                    </div>
                ))  
            )}
        </div>
